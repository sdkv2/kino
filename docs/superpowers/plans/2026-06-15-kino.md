...
```bash
npm run build
cd ~/kino && mkdir -p demo/brands/evidentcv/__none demo/assets/screens demo/specs
# minimal brand + one screenshot + a 2-segment spec (avatar + app) for the smoke test
```
Create `demo/brands/evidentcv/brand.json`, copy any PNG to `demo/assets/screens/x.png`, and `demo/specs/s.json`:
```json
{ "brand": "evidentcv", "title": "smoke", "segments": [
