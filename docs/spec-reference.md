...
| `fontWeights` | int[] 100–900 | — | Extra cuts of the brand font to stage, so `font-weight` in a motion page selects a real face instead of silently reusing the single caption cut. The caption weight is always included. **Overrides** brand `fontWeights` rather than merging — pass `[]` to opt a lean spec out of a type-heavy brand's set. Each cut is base64-inlined into every raster, so an unused one costs render time on **every frame** — the build scans your motion sources and drops any declared cut none of them reference, warning when it does. A source that sets `font-weight` in a way that can't be read statically (`var(--w)`, `` `font-weight:${w}` ``, the `font:` shorthand) makes the set unprovable and nothing is dropped. Every staged cut is also **glyph-subset** to the characters the spec can render (printable ASCII, plus every literal and HTML entity in the spec, brand and motion sources) — Google Fonts subsets server-side via `text=`, so this costs no dependency and roughly halves each cut. A bonus: `text=` reaches glyphs the default Latin cut omits, so Greek/symbol characters render in the brand face instead of silently falling back. Set `KINO_NO_FONT_SUBSET=1` to stage full cuts if a piece renders characters nothing can see statically. |
