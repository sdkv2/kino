...
| `backgroundTextures` | (string \| `{source, frames}` \| `{source, param}`)[] | — | Up to 4 texture channels for a shader background (`uTex0`..`uTex3`): image paths upload as-is; `.html` files are sanitized + rasterized DOM (fonts/palette apply). `{source, param: "name"}` **live-scrubs** the html's 1s CSS `@keyframes` per frame from that background param — smoothest, use for motion. `{source, frames: N}` bakes a static flipbook atlas (`kinoTexFrame`/`kinoTexFrameLerp`) — fine for pose-snapping, steppy for continuous motion. Shader backgrounds only. |
...
| `uTexSize0`..`uTexSize3` | `vec2` | one frame's size in css px; `(0,0)` when the channel is unbound |
| `uTexFrames0..3` / `uTexGrid0..3` | `float` / `vec2` | flipbook frame count + atlas grid (1 / 1×1 for static) |
...
pick a playback mode:

- **Live scrub (recommended)** — `{ "source": "motion/card.html", "param": "fill" }`: each frame the
  engine re-rasterizes the markup at the current value of the `fill` background param (0..1 →
  animation time), cached by value, before the frame is captured. The shader just samples
  `texture(uTex0, uv)` — the pixels already match the frame. True per-frame motion, no stepping;
  drive `fill` with `backgroundKeyframes` like any param.
- **Flipbook** — `{ "source": "...", "frames": N }` (N ≤ 64): N scrubbed snapshots baked into one
  atlas at load. Sample with `kinoTexFrame(uTexN, uTexGridN, uTexFramesN, uv, frame)` or the
  crossfading `kinoTexFrameLerp`. Cheaper per frame, but continuous motion ghosts/steps — use for
  discrete pose swaps, not slides.
