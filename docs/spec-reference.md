...
| `voiceModel` | string | `eleven_multilingual_v2` | ElevenLabs TTS model. Set `eleven_v3` to enable inline audio tags (`[excited]`, `[whispers]`, `[short pause]`, …) in segment `text`; tags direct the read and are stripped from word-synced captions. |
