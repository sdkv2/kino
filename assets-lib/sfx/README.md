...
Brand-neutral sound effects resolvable from any spec by bare id: `"src": "click"` →
`assets-lib/sfx/click.mp3` (`.mp3` preferred, `.wav` accepted). Path-like refs
(`"sfx/hit.mp3"`) resolve from the project's own `assets/` instead.

Ships **empty** — no bundled SFX. Add your own before referencing a bare id in a spec;
until then, `kino doctor` will warn "sfx library empty" and any bare-id `sfx[].src` fails
the build (use a project asset path instead, e.g. `"sfx/hit.mp3"`).
...
Run `kino audio-markers <file>` on the VO track or a
