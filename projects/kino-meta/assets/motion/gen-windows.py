#!/usr/bin/env python3
"""Emit the kino-window motion graphics from one shared chrome + tech background."""
import pathlib

HERE = pathlib.Path(__file__).parent

# Shared: TECH_BG (seam-safe animated ground) + WIN chrome.
# edge = sin(progress*π) → 0 at beat start/end so first frame ≡ last settle frame.
# Mid-beat life uses edge * f(t); never Math.random.
SHARED = r"""
function TECH_BG() {
...
# --- beat 0: type env.words. Camera peaks mid-beat, native at both ends (seamless cuts). ---
PROMPT = SHARED + r"""
...
// sin(π·progress) → 0 at start/end so the cut into beat 1 isn't a zoom pop
var breath = Math.sin(env.progress * Math.PI);
var S = 1 + 0.06 * breath;
var panY = -1.2 * breath;
var panX = 0.6 * breath * Math.sin(env.progress * Math.PI);
var cam = "translate(" + panX.toFixed(2) + "vw," + panY.toFixed(2) + "vw) scale(" + S.toFixed(4) + ")";
return WIN(out, caretOn, cam, "");
"""

# --- thinking (unused in current advert; kept for harnesses) ---
THINKING = SHARED + r"""
...
var caretOn = started
  ...
  : false;
// Soft mid-beat breath; native at ends (matches prompt/settle framing)
var breath = Math.sin(env.progress * Math.PI);
var S = 1 + 0.05 * breath;
var panY = -0.8 * breath;
...
# --- loop-settle: hold native framing (matches every beat edge) + fade check ---
SETTLE = SHARED + r"""
var p = env.progress;
