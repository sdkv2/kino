...
export async function generate(lookId, audioAssetId) {
    ...
    if (d.error)
        throw new Error(`HeyGen generate: ${d.error.message}`);
    return d.data.video_id;
}
export async function pollDownload(videoId, out, timeoutSec = 600) {
    ...
    while (Date.now() < deadline) {
        ...
        if (st === "failed")
            throw new Error(`HeyGen video failed: ${JSON.stringify(d.data?.error)}`);
        await new Promise((r) => setTimeout(r, 12000));
    }
    throw new Error("HeyGen timed out");
}
// --mock: a 6s silent placeholder so render runs with zero spend.
export async function generateMock(out) {
    await execa("ffmpeg", [
        "-y", "-loglevel", "error",
