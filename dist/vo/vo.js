...
const GAP = 0.32;
export function computeTimings(durations, gap) {
    const out = [];
    let t = 0;
...
export async function buildVO({ spec, voiceId, cache, apiKey, mock }) {
    ...
    for (const [i, seg] of spec.segments.entries()) {
        ...
        if (!clip) {
            ...
            if (mock)
                await ttsMock(seg.text, tmp);
            else
                await tts(apiKey, voiceId, seg.text, tmp);
