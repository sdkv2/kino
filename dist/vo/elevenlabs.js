...
export async function listVoices(apiKey) {
    ...
    if (!r.ok)
        throw new Error(`ElevenLabs voices ${r.status}`);
...
export async function tts(apiKey, voiceId, text, out, settings = DEFAULT_SETTINGS) {
    ...
    if (!r.ok)
        throw new Error(`ElevenLabs TTS ${r.status}: ${await r.text()}`);
    writeFileSync(out, Buffer.from(await r.arrayBuffer()));
}
// --mock: ~0.4s/word of silence so timing math + render still work with zero spend.
export async function ttsMock(text, out) {
