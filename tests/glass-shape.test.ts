...
import { lerpPathD, samplePathAnimate } from "../src/render/native/page/glassShape.js";
...
describe("kino-glass SVG shape", () => {
  ...
  it("allows SMIL only in kino-glass-shape markup", () => {
  ...
  it("star silhouette differs from a round-rect card at the corners", async () => {
    ...
    const rect = star.replace(
      /<svg class="kino-glass-shape"[\s\S]*?<\/svg>/,
      "",
