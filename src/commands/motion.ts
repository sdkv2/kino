...
export function motionHelpText(): string {
  ...
  return [
    ...
    "  · env = { frame, t, progress, pulse, params, palette:{mint,green,night,white,gold,font},",
    "           width, height, words? }  (words = beat-relative VO timings when the beat has speech).",
    ...
    "  · env has no duration field; end-of-beat → env.progress > ~0.95 (progress never equals 1.0).",
    "",
    "Camera zoom/pan: CSS transform on a wrapper (scale/translate driven by --progress, a keyframed",
    "param, or typed-char fraction from env.words). Motion graphics do NOT use zoomKeyframes (that",
    "track is for app footage + frame chrome). Zooming a motionOverlay on a static PNG desyncs text",
    "from chrome — put chrome+text in one motion beat. See skills/speech-synced-ui.",
    "",
