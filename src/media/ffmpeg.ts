...
export async function trimAudio(src: string, endSec: number, out: string): Promise<void> {
  await execa("ffmpeg", ["-y", "-loglevel", "error", "-i", src, "-t", String(endSec), out]);
}
...
export async function stitchAudio(clips: string[], gapSec: number, out: string): Promise<void> {
  ...
  await execa("ffmpeg", ["-y", "-loglevel", "error", "-f", "lavfi", "-i", "anullsrc=r=44100:cl=mono",
    "-t", String(gapSec), sil]);
  ...
  for (const [i, c] of clips.entries()) {
    ...
    await execa("ffmpeg", ["-y", "-loglevel", "error", "-i", c,
