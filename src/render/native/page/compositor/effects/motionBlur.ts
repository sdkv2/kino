...
export const motionBlurPass: EffectPass = {
  name: "motionBlur",
  uniformNames: ["uAngle", "uDistance", "uSamples"],
  frag: `
...
void main() {
  ...
  if (uDistance <= 0.0) { kino_frag = texture(uSrc, uv); return; }

  vec2 texel = 1.0 / uRes;
  vec2 dir = vec2(cos(uAngle), sin(uAngle)) * uDistance * texel;
  int n = uSamples;
  vec4 sum = vec4(0.0);
  ...
  uniforms(gl, loc, params) {
    const distance = Math.min(Math.max(0, Number(params.distance ?? 0)), MAX_DISTANCE);
    const angleDeg = Number(params.angle ?? 0);
    const angleRad = (angleDeg * Math.PI) / 180;
    const samples = Math.min(
      MAX_SAMPLES,
      Math.max(1, Math.round(Number(params.samples ?? DEFAULT_SAMPLES))),
    );
