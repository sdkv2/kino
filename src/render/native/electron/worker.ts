...
function writeErr(msg: string): void {
  process.stderr.write(`[electron worker] ${msg}\n`);
  process.exit(1);
}

/** Per-request failure: reject that one command and keep serving.
 *
 *  This used to call writeErr, which exits the process — so a page fault the caller was entitled
 *  to see and handle (a shader that will not compile is the obvious one) arrived as "electron host
 *  exited code=1", with the real message stranded on stderr. Newlines are escaped because the
 *  reply protocol is line-oriented. */
function writeError(id: number, msg: string): Promise<void> {
  return withStdout(() => {
    process.stdout.write(`E ${id} ${msg.replace(/\r?\n/g, "\\n")}\n`);
  });
}

function enqueueSlot(slot: number, id: number, fn: () => Promise<void>): void {
  const prev = chains.get(slot) ?? Promise.resolve();
  const next = prev.then(fn).catch((e: unknown) =>
    writeError(id, errText(e)).then(
      () => {},
      () => {},
    ),
  );
...
function enqueueGlobal(fn: () => Promise<void>): void {
  globalChain = globalChain.then(fn).catch((e: unknown) => writeErr(errText(e)));
}
